    ...
    var body: some View {
        VStack {
            ...
            Image(systemName: "music.note")
                .font(.system(size: 85))
                .padding(.bottom)
            Text("Start seraching for music...")
                .font(.title)
            Spacer()
        }
...
struct SearchBar: UIViewRepresentable {
    ...
    func makeCoordinator() -> SerachBarCoordinator {
        return SerachBarCoordinator(searchTerm: $searchTerm)
    }
    
    class SerachBarCoordinator: NSObject, UISearchBarDelegate {
        @Binding var searchTerm: String
